 ...
 */
workbench.get("/api/workbench/teams/:teamId/overview", requireWorkbenchAuth, async (c) => {
  const teamId = await resolveWorkbenchTeamId(c.env.KV, c.req.param("teamId"));
  ...
  const [setsRow, totalRow, completedRow, claimedRow] = await Promise.all([
    ...
    claimSupported
      ? c.env.DB.prepare(
        ...
        ).bind(teamId, session.userId).first<{ count: number }>()
 ...
 */
workbench.get("/api/workbench/teams/:teamId/sets", requireWorkbenchAuth, async (c) => {
  const teamId = await resolveWorkbenchTeamId(c.env.KV, c.req.param("teamId"));
  const session = c.get("session");
  const role = getTeamRole(session, teamId);
  if (!role) return c.json({ error: "Not a member of this team" }, 403);
 ...
 */
workbench.get("/api/workbench/teams/:teamId/my-todos", requireWorkbenchAuth, async (c) => {
  const teamId = await resolveWorkbenchTeamId(c.env.KV, c.req.param("teamId"));
  const session = c.get("session");
  const role = getTeamRole(session, teamId);
  if (!role) return c.json({ error: "Not a member of this team" }, 403);
 ...
 */
workbench.get("/api/workbench/teams/:teamId/feed", requireWorkbenchAuth, async (c) => {
  const teamId = await resolveWorkbenchTeamId(c.env.KV, c.req.param("teamId"));
  const session = c.get("session");
