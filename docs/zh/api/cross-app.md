...
|                   | `* /api/cross-app/teams/:teamId/sets/:setId/ws`（WebSocket 升级）|
...
|                   | `GET /api/cross-app/teams/:teamId/members`（`read_todos`）      |
|                   | `GET /api/cross-app/teams/:teamId/assigned-to-me`（`read_todos`）|
|                   | `GET /api/cross-app/assigned-to-me`（`read_todos`，跨所有工作区）|
...
返回的待办列表是扁平结构。子待办的 `parentId` 指向其父待办。被分配人的 `name` / `username` / `avatarUrl` 在团队空间中通过 Prism 解析；个人空间下仅解析当前调用者。

---

## `GET /api/cross-app/assigned-to-me`

返回分配给调用方的所有**跨工作区**未完成待办，按工作区划分。衍生应用可用此接口构建跨
工作区的"我的工作"视图。

**所需 scope：** `read_todos`（无团队路径参数——解析令牌用户并查询其全部成员身份）。
